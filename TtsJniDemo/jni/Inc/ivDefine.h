...
typedef struct tagWatchExtInfo ivTWatchExtInfo, ivPtr ivPWatchExtInfo;
struct tagWatchExtInfo
{
    ivPCUInt8 		pPoS;		/* POS */

    ivUInt32		nStarTime;	/* dur(ms) of syl */
...
struct tagResPackDescExt
{
    ...
    ivUInt32		nBCheck;			/* resource check (0 for No, 1 for yes, 2 for simple, if check, nSize must be fill in) */
...
#define AISOUND_UNREFERENCED_PARAMETER(P)          (P)
